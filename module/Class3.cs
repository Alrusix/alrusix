...
namespace akronWS
{
	...
	{
		static List<Socket> clientSockets { get; set; } = new List<Socket>();
		public static void Start()=> _ = Task.Run(() => ww()); 
		static List<Socket> checkRead = new List<Socket>();
		static List<Socket> checkWrite = new List<Socket>();
		static List<Socket> checkError = new List<Socket>();
		readonly static string wwe =$"Welcome   {"\r\n"}";
		
		public static  void ss(Socket socket)
		{
			clientSockets.Add(socket);		
			socket.Send(EncodeMessage(wwe));
		}
		static async void ww()
		{
			while (true)
			{
		...
		private static int DecodeMessage(byte[] buffer, int length,ref string ww)
		{
		...
		public static void AcceptMessage(List<string> bytes)
		{
		...
		private static byte[] EncodeMessage(string message, int opcode=0x1)
		{
