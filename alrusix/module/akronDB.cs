...
namespace akronDB
{
	...
	{	
		static readonly string dbPath = new(Config.Get<string>("DB:Path", "akron.adb"));
		static List<Tables> TableList { get; set; } = Init();
		...
		static DateType Type_to_DateType(string type)
		{
		...
		static Type DateType_to_Type(DateType type) => type switch
		{
		...
		public static string Serialize()=> JsonSerializer.Serialize(DBEngine.TableList,new JsonSerializerOptions { WriteIndented =true});	
		...
		static List<Column>? CheckFields(string tableName)
		{
		...
		public static Tables? GetTable(string tableName) => TableList.SingleOrDefault(t => t.Name == tableName);
		public static bool Check(string tableName) => TableList.Any(t => t.Name == tableName);
		...
		static byte ParseCreateTableStatement(string sql)
		{
			...
			{
				SocketServer.logger.Log($"{ex.StackTrace}|{ex.Message}|{ex.Source}",2);
				return 0x4;
			}			
		}
		//TODO 
		...
		public static byte ParseSQL(string sql)
		{
			...
			foreach (string s in w)
			{
				...
				{
					case "CREATE":
						return ParseCreateTableStatement(s);
					case "INSERT":
						return InsertIntoDatabase(s);
					case "UPDATE":
		...
		/// <returns></returns>
		static byte ParseUPDATE(string sql)
		{
			try
			{
			...
			{
				SocketServer.logger.Log($"StackTrace:{ex.StackTrace}|Message:{ex.Message}|Source:{ex.Source}", 2);
		...
		/// <param name="sql"></param>
		/// <returns></returns>
		static byte ParseDELETE(string sql)
		{
			try
			{
		...
		/// <param name="sql"></param>
		/// <returns></returns>
		static byte ParseALTER(string sql)
		{
			try
			{
		...
		/// <param name="sql"></param>
		/// <returns></returns>
		static byte ParseDROP(string sql)
		{
			try
			{
				...
				if (!match.Success)
				{
					//SocketServer.logger.Log("SQL语法错误: 无法匹配到DROP语句", 2);
					return (byte)SqlCode.FormatError;
				}
				string tableName = match.Groups[1].Value;
				Tables? table = GetTable(tableName);
		...
		/// 将选中的字段返回给用户。
		/// </summary>
		static byte ParseSELECT(string sql)
		{
			try
			{
				sql = sql.Replace("\n", "").Replace("\r", "").Trim();
		...
		//TODO
		static bool IsConstraintKeyword(string word)
		{
		...
		//	}
		//}
		static void ShowTable(string[] head, List<Dictionary<string, object?>> tableData)
		{	
			if (tableData.Count > 0)
			{
				Program.Options.Add(new UP());
		...
		/// </summary>
		static string GetTruncatedString(string text, int maxDisplayWidth)
		{
			int currentWidth = 0;
			int endIndex = 0;
		...
		/// 根据显示宽度进行填充，确保显示时对齐
		/// </summary>
		static string PadString(string text, int totalWidth)
		{
